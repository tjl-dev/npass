...
export class nPassTokenVideo extends Component {
    ...
    onTokenReceived(tokenResponse) {
        ...
        this.fetchVideo(this.props.contentDetails.src, tokenResponse.tokenData.token)
    }

    fetchVideo(url, token) {
        fetch(url, {
            method: 'GET',
        ...
        .then(video => {
            var videoObjectUrl = URL.createObjectURL(video)
            this.setState({videoObjectUrl, active: true})       
    ...
    render(){
        ...
        const controls = this.props.contentDetails.showControls || true
        const hasExtension = this.state.hasExtension

        const active = this.state.active
        const videoObjectUrl = this.state.videoObjectUrl      
        ...
        return (
            <div style={{display: 'grid', margin: '10px'}} >
                <div className={styles.nPassContentWrapper} >
                    <div >
                        { active ? (
                                <div>
                                    <div className={styles.nPassContentWrapper} >
                                        <video src={videoObjectUrl} style={style} autoplay="autoplay" id="videoObject" controls={controls} />
                                    </div>
                                </div>
                            ) : (
                ...
                { active == false &&
                <div style={{display: 'grid', margin: '10px', position: "absolute", right: "36.6%", top: "70%"}}>
...
export default nPassTokenVideo;
